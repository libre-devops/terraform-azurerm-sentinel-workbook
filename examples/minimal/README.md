...
The smallest valid call: one watchlist with a couple of native rows on a freshly onboarded
workspace, wired through the sentinel module's onboarding_id. The environment comes from the
Terraform workspace (`terraform.workspace`), not a variable. Run it with `just e2e minimal`, which applies the stack then always destroys it.
...
| Name | Description |
|------|-------------|
| <a name="output_workbook_ids"></a> [workbook\_ids](#output\_workbook\_ids) | Map of workbook label to id. |
<!-- END_TF_DOCS -->
